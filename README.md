...
**`paper/tabla2.csv` holds these eight rows at full precision**, written by
`resultados.py` from the same file that renders the table in the paper, so it is
the one place to look. The per-model files it consolidates are
`results/classics_cv_comparison.csv`, `results/dl_comparison.csv`,
`results/agente_comparison.csv` and `results/cost_table.csv`.
