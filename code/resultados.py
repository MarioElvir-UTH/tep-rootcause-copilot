...
Reads:  results/tabla2.json, results/effect_sizes.csv, results/per_fold_f1.csv,
        results/human_load.csv, results/rubrica_humana/acuerdo.csv
Writes: paper/tabla2.tex, paper/tabla2.csv, paper/resultados.md,
        paper/resultados.tex, paper/rubrica.tex, those three
        regions of the manuscript, and the expected-results table of the
        README
"""
import io
import os
...
CSV = os.path.join(BASE, "paper", "tabla2.csv")
...
if CHECK:
    ...
    FILES_OK.append(compare(CSV, tabla_csv, "paper/tabla2.csv"))
...
if CHECK and not HAS_TEX:
    ...
    print("wrote paper/tabla2.tex and paper/tabla2.csv")
    if not HAS_TEX:
        pass
    elif same:
...
# -------------------------------------------------- the human rubric, in one sentence
# It reached no agreement beyond chance, and that is the result. The numbers come
# from 24_acuerdo_rubrica.py, which is not part of run_all.py, so the region is
# skipped when acuerdo.csv is not here instead of crashing.
ACU = os.path.join(BASE, "results", "rubrica_humana", "acuerdo.csv")
RBEGIN = "% BEGIN rubrica (generated by resultados.py; do not edit by hand)"
REND = "% END rubrica"
if not os.path.isfile(ACU):
    print("nota: results/rubrica_humana/acuerdo.csv no esta; el parrafo de la rubrica se omite")
else:
    ac = pd.read_csv(ACU).set_index("item")
    cubren = ((ac["ic95_bajo"] <= 0) & (ac["ic95_alto"] >= 0)).all()
    R = [
     "A human rubric of %s binary items, scored on %d sampled episodes by two raters "
     "who did not confer, reached no agreement beyond chance on any item, with Cohen's "
     "$\\kappa$ from $%+.2f$ to $%+.2f$ and %s 95\\%% interval containing zero, so it "
     "measures the rubric and not the explanations."
     % (EN_PALABRAS(len(ac)), int(ac["n"].iloc[0]), ac["kappa"].min(),
        ac["kappa"].max(), "every" if cubren else "not every"),
    ]
    ...
    had = tex[i + len(RBEGIN):j].strip() if HAS_TEX else rub.strip()
    if CHECK and HAS_TEX:
        okr = had == rub.strip()
        print("el parrafo de la rubrica %s lo que generan los datos"
              % ("COINCIDE con" if okr else "NO coincide con"))
        CHECKS.append(okr)
    elif CHECK or not HAS_TEX:
        pass
    elif had == rub.strip():
        print("el manuscrito ya llevaba el parrafo de la rubrica: no se toco")
    else:
        io.open(TEX, "w", encoding="utf-8", newline="\n").write(
            tex[:i + len(RBEGIN)] + "\n" + rub + "\n" + tex[j:])
        print("manuscrito actualizado con el parrafo de la rubrica")
